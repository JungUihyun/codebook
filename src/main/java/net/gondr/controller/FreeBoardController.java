...
public class FreeBoardController {
	...
	public String writeProcess(FreeBoardVO board, HttpSession session, Errors errors, RedirectAttributes rttr) {
		// 올바른 값인지 벨리데이팅
		new BoardValidator().validate(board, errors);
		if (errors.hasErrors()) {
			return "freeBoard/write"; // 에러가 존재하면 글쓰기 페이지로 보냄.
		}
		// 여기는 인터셉터에 의해서 로그인하지 않은 사용자는 막히게 될 것이기 때문에 그냥 에러처리 없이 user를 불러써도 된다.
		UserVO user = (UserVO) session.getAttribute("user");

		if (board.getId() != null) {
			FreeBoardVO data = service.viewArticle(board.getId());
	...
	public String viewList(Criteria criteria, Model model) {
		...
		List<FreeBoardVO> list = service.getArticleList(criteria);
	...
	public String viewModPage(Model model, @PathVariable("id") Integer id, HttpSession session,
		...
		FreeBoardVO data = service.viewArticle(id);
		UserVO user = (UserVO) session.getAttribute("user");
		...
		model.addAttribute("freeboardVO", data);
		return "freeBoard/write";
	}
	...
	public String deleteArticle(@PathVariable("id") Integer id, HttpSession session, RedirectAttributes rttr) {
		...
		if(!user.getUserid().equals(data.getWriter())) {
			...
			return "redirect:/free/view/" + data.getId();
		}
